...
module.exports = async ctx => {
  ...
  let comments = await mysql.raw('SELECT cid,comment.pid,user.iavatar, name, comment.time, text_comm,state FROM comment, user ,nickname,post WHERE comment.uid = user.uid AND state=0 AND comment.nid = nickname.nid AND post.uid = ? ORDER BY time DESC', [uid])
